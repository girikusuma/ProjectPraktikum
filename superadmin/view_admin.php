          ...
          <section class="connectedSortable">
            <table class="table">
              ...
              <?php
                  ...
                  $next_page = $page + 1;
                  $prev_page = $page - 1;
                  $result = $connection->query("SELECT * FROM tb_users u JOIN tb_rules r ON u.id_rule=r.id_rule WHERE u.id_rule='2'");
                  $total_records = mysqli_num_rows($result);
                  $total_no_page = ceil($total_records/$total_data_page);
                  $second_last = $total_no_page - 1;
                  $resultPerPage = $connection->query("SELECT * FROM tb_users u JOIN tb_rules r ON u.id_rule=r.id_rule WHERE u.id_rule='2' LIMIT $offset,$total_data_page");
              ?>
              <?php $no = $offset+1; ?>
              <?php  while($row = mysqli_fetch_assoc($resultPerPage)) : ?>
                  <tr>
                      ...
                      <?php } ?>
                      <td>
                        <a href="hapus_admin.php?id=<?=$row['id_user']?>"><button type="button" class="btn btn-danger">Hapus</button></a>
                      </td>
                  </tr>
                  <?php $no++; ?>
                  <?php endwhile; ?>
